...
- The 5 fixed quick-access icons (🎒 Backpack, 🛠️ Camp Workbench, ➕ First Aid, 🐻🚫 Stop Bear, 🖐️ Use Hand) stay on screen same as always, but the full 9-slot hotbar itself is hidden on touch — it's sized for a desktop-width screen and would overlap the joystick and action buttons the same way the four buttons below used to. Change what you're holding through the Inventory screen instead (see below) — there's effectively one held item on touch rather than nine hotbar slots to flip between.
...
Your inventory — everything you're currently holding, with live counts — is saved to this browser. Open it with `I`, the **📦 Inventory (I)** button (the **☰** menu on touch), or by clicking a hotbar slot that's already selected. It's split into what you actually have ("Your items", with a count on each) and everything else you could still obtain or craft ("Not yet obtained", grayed out) — tap any tile, held or not, to put it in the currently-selected hotbar slot.
...
- **Fishing Pole** — right-click it into any water with a fish nearby to cast your line, then hold
  ...
  the ninth-to-last one on the sash.
- **Kayak** — moored right at the water's edge on the lake near camp. Walk or swim up to it and
  ...
  eighth-to-last one on the sash.
- **Horse** — tied up in a free corner of the cooking area. Walk up and you're mounted automatically,
  ...
  the Horseback Riding badge, the seventh-to-last one on the sash.
- **Diving mask & snorkel** — no crafting needed, they just show up on your character the moment your
  ...
  earns the Scuba Diving badge, the fifth-to-last one on the sash.
- **Bow** — craftable at the workbench like the Fishing Pole, and just as much a tool as a material:
  right-click to loose an arrow along wherever you're looking, a real 3D shaft with fletching that
  actually flies (gravity included, so longer shots drop a little) rather than an instant hitscan.
  It only works at the Archery Range (see [Archery Range](#archery-range) below) — anywhere else,
  right-clicking it just tells you so instead of firing. Hitting a target 5 times earns the Archery
  badge, the third-to-last one on the sash.
...
the real text it recites, rather than a handful of symbols repeating, each ring carved with its own
small icon and no lettering (the block art is too small to actually render words legibly, so this
is deliberately icon-only rather than trying and failing to be readable): the **Scout Oath Totem**,
4 rings tall, reads bottom to top "On my honor" (a blank bronze plaque), the Physically Strong/
Mentally Awake/Morally Straight creed (a hiker/book/pillar trio), "To obey the Scout Law" (a compass
flanked by two scouts), and "Duty to God and Country" (a cross and shield). The **Outdoor Code
Totem**, 5 rings tall, reads the same way bottom to top — an "As an American..." recap (the same
blank plaque icon), then the Code's own 4 points in recitation order: Conservation (a pine tree),
Considerate (a hiker and peak), Fire (a ringed campfire), and Clean (a broom). Both totems are topped
with the same perched eagle, wings spread (the model the ambient Giant Eagles use, just posed still),
and both hang a small always-lit lantern for a bit of warm glow after dark. Right-click either one —
ordinary reach applies, no special long-range check needed at this height — and it plays a full
...
A second flat, permanent clearing sits flush against the cooking area's south edge, same fixed
elevation as the rest of camp — a 10x16 lane framed by four short wooden corner posts, with 3 straw
targets side by side at the far end, each 2 blocks tall on its own post so it sits roughly at eye
height and gives a level shot some real room for its natural gravity drop over the lane's length.
Every target face is a genuine hand-drawn bullseye — concentric white/black/white/red/gold rings —
not a flat color swatch.

Craft a **Bow** at the workbench (2 Stick, 1 Rope, same scale as the Fishing Pole's recipe) and
right-click to loose a real 3D arrow, gravity and all, along wherever you're looking — arrows are
unlimited once you have the Bow, nothing to run out of mid-practice. The catch: it only fires inside
...
If a bear (or a moose that's turned on you) is already on you, the **🐻🚫 Stop Bear** quick-access icon (bottom of the screen, next to the hotbar) is the emergency out: click it to shout and clap — a big **GO AWAY! BEAR! GO AWAY!** banner on screen, a burst of deliberately silly noise — and anything actually dangerous within about 8 blocks (bear or moose, anything else can't hurt you in the first place) turns and runs for a full 6 seconds, aggro cleared, so it doesn't just spin around and resume the charge the moment the fright wears off. It's on a 3-second cooldown so it's a real emergency tool, not a way to make wildlife harmless outright.
...
- Best played on a desktop or laptop with a keyboard and mouse — pointer lock and WASD aren't a good fit for touch screens. It also works on phones and tablets with on-screen touch controls, but that's not what it was designed for — and a phone specifically has to be held in landscape, or a "please rotate" screen blocks play until it is.
